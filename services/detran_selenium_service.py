...
class DetranService(SeleniumInterface):
    ...
    def run(self) -> str:
        ...
        popup_button = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.XPATH, self.popup)))
        popup_button.click()
        ...
        time.sleep(3) #alterar depois no async
    ...
    def prepare(self, parameters : dict) -> str:
        ...
        self.url = self.config["detran"]
        self.popup = self.config["popup_id"]
        self.placa_field_id = self.config["detran_placa_field_id"]
        self.renavam_field_id = self.config["detran_renavam_field_id"]
        self.wait_element = self.config["detran_presence"]
        self.wait_ready_page_element = self.config["detran_ready_page_element"]
